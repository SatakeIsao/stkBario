...
namespace app
{
	namespace collision
	{
            ...
            if (isRequestTeleport_) {
                ...
                // XZ平面（壁）の移動解決
                {
                    ...
                    while (true) {
                        ...
                        end.setOrigin(btVector3(intendedXZPos.x, posTmp.y, intendedXZPos.z));

                        SweepResultWall callback;
                        callback.me = rigidBody_.GetBody();
                        callback.startPos = posTmp;

                        PhysicsWorld::GetInstance()->ConvexSweepTest((const btConvexShape*)collider_.GetBody(), start, end, callback);
                ...
                if (verticalVelocity_ > 0.0f) {
                    ...
                    btTransform start, end;
                    start.setIdentity(); end.setIdentity();
                    start.setOrigin(btVector3(position_.x, checkY, position_.z));
                    end.setOrigin(btVector3(position_.x, checkY + upAmount, position_.z));
                ...
                else
                {
                    ...
                    start.setIdentity(); end.setIdentity();
                    start.setOrigin(btVector3(position_.x, checkY, position_.z));
                    end.setOrigin(btVector3(position_.x, checkY - checkDist, position_.z));
        ...
        {
            PhysicsWorld::GetInstance()->RemoveRigidBody(rigidBody_);
        }
