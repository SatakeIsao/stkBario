...
namespace nsK2EngineLow {
    ...
    {
        if (initData.m_noTexture) {
            m_numTexture = 0;
            return;
        }
        // スプライトで使用するテクスチャを初期化する。
        if (initData.m_ddsFilePath[0] != nullptr) {
            // ddsファイルのパスが指定されているならddsファイルからテクスチャを作成する。
            int texNo = 0;
    ...
    {
        // ピボット値に基づいた平行移動行列を作成。
        // ピボットは中心が0.5, 0.5。左上が0.0, 0.0、右下が1.0, 1.0になるようにする。
